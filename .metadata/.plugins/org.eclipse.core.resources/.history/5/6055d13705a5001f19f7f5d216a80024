...
public class AfiliadoRepositoryCustomImpl implements AfiliadoRepositoryCustom {
	...
	public AfiliadoRepositoryCustomImpl(EntityManager entityManager) {
	...
	public boolean existsByTipoIdentificacionIdAndNumeroIdAfiliado(UUID tipoIdentificacionId, String numeroIdAfiliado) {
	    try {
	        ...
	        var query = criteriaBuilder.createQuery(Long.class);
	        var root = query.from(AfiliadoEntity.class);
	        var tipoIdentificacion = root.get("tipoIdentificacion");

	        query.select(criteriaBuilder.count(root))
	             .where(criteriaBuilder.and(
	                 criteriaBuilder.equal(tipoIdentificacion.get("id"), tipoIdentificacionId),
	                 criteriaBuilder.equal(root.get("numeroIdAfiliado"), numeroIdAfiliado)
	             ));
	        ...
	        throw RepositoryDeviUcoException.create(
	            "Error al verificar si existe un afiliado con el mismo tipo y número de identificación", 
	            null, 
	            exception
	        );
	    }
	...
	public boolean existsByTelefono(String telefono) {
	    try {
	        ...
	        query.select(criteriaBuilder.count(root))
	             .where(criteriaBuilder.equal(root.get("telefono"), telefono));
	        ...
	        throw RepositoryDeviUcoException.create(
	            "Error al verificar si existe un afiliado con el mismo teléfono", 
	            null, 
	            exception
	        );
	    }
